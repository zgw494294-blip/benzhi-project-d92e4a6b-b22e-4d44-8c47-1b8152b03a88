...
func (a *API) SubmitMeasurementHandler(writer http.ResponseWriter, request *http.Request, caseID string) {
	if !requireRole(writer, request, "certification_engineer") {
		return
	}
	var command certification.SubmitMeasurementCommand
	...
	value, err := a.service.SubmitMeasurement(caseID, command)
	if err != nil {
